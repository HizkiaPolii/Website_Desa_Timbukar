...
export default function EditAgamaPage() {
  const [items, setItems] = useState<ReligionItem[]>([]);
  ...
  const loadData = async () => {
    try {
      ...
      if (json?.data?.religion) setItems(json.data.religion);
    } catch (err) {
      console.error(err);
      setError("Gagal memuat data agama");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAdd = () =>
    setItems((prev) => [...prev, { agama: "", jumlah: "", persentase: "" }]);
  ...
  const handleSave = async () => {
    try {
      ...
      showToast.success("Distribusi agama berhasil diperbarui");
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold">Edit Distribusi Agama</h1>
            ...
            >
              <div>
                ...
                <input
                  ...
                  value={it.agama}
                  onChange={(e) => handleChange(idx, "agama", e.target.value)}
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
