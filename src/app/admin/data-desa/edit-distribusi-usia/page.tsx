...
export default function EditDistribusiUsiaPage() {
  const [items, setItems] = useState<DemographicsItem[]>([]);
  ...
  const loadData = async () => {
    try {
      ...
      if (json?.data?.demographics) setItems(json.data.demographics);
    } catch (err) {
      console.error(err);
      setError("Gagal memuat distribusi usia");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAdd = () =>
    setItems((prev) => [
      ...prev,
      { kategori_usia: "", jumlah: "", persentase: "" },
    ]);
  ...
  const handleSave = async () => {
    try {
      ...
      showToast.success("Distribusi usia berhasil diperbarui");
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold">Edit Distribusi Usia</h1>
            ...
            >
              <div>
                ...
                <input
                  ...
                  value={it.kategori_usia}
                  onChange={(e) =>
                    handleChange(idx, "kategori_usia", e.target.value)
                  }
                />
              </div>
              <div>
